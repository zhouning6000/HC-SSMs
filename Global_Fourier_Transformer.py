...
class AdativeFourierNeuralOperator(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.fft.rfft2(x, dim=(1, 2), norm='ortho') #torch.fft.rfft2专门用于实数输入的二维快速傅里叶变换 (2D FFT)。它只计算和返回正频率部分的结果，从而减少计算量和内存占用。(batch_size, M, N)->(batch_size, M, N//2 + 1)
        ...
        x = torch.view_as_complex(x) #假设有一个形状为 (m, n, 2) 的实数张量 x，这个张量的最后一个维度大小为 2，它代表了复数的实部和虚部。torch.view_as_complex(x) 将输出一个形状为 (m, n) 的复数张量，每个元素是 x 中对应位置上的复数。
        x = x.reshape(B, x.shape[1], x.shape[2], self.hidden_size) #得到傅里叶数
        x = torch.fft.irfft2(x, s=(self.h, self.w), dim=(1,2), norm='ortho') #（50,4,3,768）复数 变成了（50,4,4,768）
        x = x.reshape(B, N, C)#（50,16,768）
