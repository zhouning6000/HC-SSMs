...
class HierarchicalModel(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        #self.enc_embedding = DataEmbedding(input_dim, model_dim, embed, freq,dropout)
        self.low_level = S4Model(input_dim=input_dim,output_dim=output_dim,model_dim=model_dim)
        self.high_level = MambaNet(model_dim=output_dim,nlayers=6)

        self.src_mask = None
        self.pos_encoder = PositionalEncoding(model_dim)  # , dropout)
        self.output = nn.Linear(model_dim, output_dim)
        self.freq_ratio = freq_ratio
        self.conv1x1 = nn.Conv2d(model_dim,model_dim*10, kernel_size=1)

    def forward(self, x, has_mask=True):
        # x= self.enc_embedding(x, batch_x_mark)


        # Reshape x into B, Lc, C, D
        x = x.reshape(x.shape[0], -1, int(self.freq_ratio), x.shape[-1])

        #proc_x = self.low_level(x.reshape(-1, *x.shape[2:]))[:, -1]# 取第2个维度的最后一个维度计算
        proc_x=self.low_level(x.reshape(-1, *x.shape[2:]))
        proc_x=proc_x[:,-1]
        #proc_x = proc_x.reshape(*x.shape[0:1], -1,*proc_x.shape[2:]) #取最后一维的数字
        ...
        # output=self.conv1x1(output.reshape(2,512,16,16))#
        # output=output.reshape(2,10,512,16,16)
        output=self.conv1x1(output.reshape(2,256,16,16))#32
        output=output.reshape(2,10,256,16,16)#10-17-moving
        return output
