...
def load_navier_stokes_data(path, sub=1, T_in=10, T_out=10, batch_size=16, reshape=None):
    ntrain = 3500
    neval=200
    ntest = 200
...
def train_model(model, train_loader, eval_loader, criterion, optimizer, device, num_epochs=150):
    ...
    for epoch in range(num_epochs):
        ...
        if eval_loss < best_loss:
            ...
            torch.save(model.state_dict(), 'best_model_weights_sevir.pth')
...
def main(args):
    ...
    model = HC_model(shape_in=(10, 1, 192, 192))
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model.to(device)
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    #model.load_state_dict(torch.load('best_model_weights_sevir.pth'))
    #model.load_state_dict(torch.load('best_model_weights_sevir.pth'))
