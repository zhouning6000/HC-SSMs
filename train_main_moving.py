...
def load_navier_stokes_data(path, sub=1, T_in=10, T_out=10, batch_size=12, reshape=None):
    ...
    data = np.load(path)
    data = torch.tensor(data, dtype=torch.float32).permute(1, 2, 3, 0)
    data=data/255.0
...
def evaluate_model(model, eval_loader, criterion, device):
    ...
    torch.save({'input': input_list_tensor, 'output': output_list_tensor, 'target': targets_list_tensor}, 'tensors.pth')
...
def train_model(model, train_loader, eval_loader, criterion, optimizer, device, num_epochs=150):
    best_loss = float('inf')
    #best_loss=evaluate_model(model, eval_loader, criterion, device)
    ...
    for epoch in range(num_epochs):
        ...
        if eval_loss < best_loss:
            ...
            torch.save(model.state_dict(), 'best_model_weights_moving_HC.pth')
...
def main(args):
    ...
    model = HC_model(shape_in=(10, 1, 64, 64))
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model.to(device)
    model.load_state_dict(torch.load('best_model_weights_moving_HC.pth'))
    #model.load_state_dict(torch.load('best_model_weights_moving.pth'))
